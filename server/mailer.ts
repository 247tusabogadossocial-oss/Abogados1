...
function getMailFrom() {
  ...
  const fromEmail = String(process.env.MAIL_FROM_EMAIL ?? "").trim() || String(process.env.SMTP_USER ?? "").trim();
...
function ensureSendGridClientReady() {
  const apiKey = String(process.env.SENDGRID_API_KEY ?? "").trim();
  if (!apiKey) throw new Error("SENDGRID_API_KEY no esta configurado");
  // Setting the API key multiple times is OK, but do it once per process if you prefer.
...
async function deliverEmail(input: {
  ...
  ensureSendGridClientReady();

  const { fromEmail, fromName } = getMailFrom();
